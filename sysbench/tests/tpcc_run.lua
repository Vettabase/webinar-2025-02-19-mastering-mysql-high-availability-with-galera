...
--  AND c_id = :c_id;
  ...
  -- Variation 1: BEGIN
    local update_amount = sysbench.rand.uniform(1, 50)  -- Reduce range to avoid integer overflow
    
    -- Fetch current w_ytd to avoid overflow
    local w_ytd = con:query_row(string.format("SELECT w_ytd FROM warehouse%d WHERE w_id = %d;", table_num, w_id))
    if w_ytd == nil then w_ytd = 0 end -- Prevent nil value

    if w_ytd + update_amount > 2147483647 then update_amount = 2147483647 - w_ytd end  -- Prevent overflow
    
    con:query(string.format("UPDATE /* Variation1 */ warehouse%d SET w_ytd = w_ytd + %d WHERE w_id = %d;", table_num, update_amount, w_id))
    con:query("SELECT SLEEP(1);")
    
    -- Fetch current d_ytd to avoid overflow
    local d_ytd = con:query_row(string.format("SELECT d_ytd FROM district%d WHERE d_id = %d;", table_num, d_id))
    if d_ytd == nil then d_ytd = 0 end -- Prevent nil value
    if d_ytd - update_amount < -2147483648 then update_amount = d_ytd + 2147483648 end  -- Prevent underflow
    
    con:query(string.format("UPDATE /* Variation1 */ district%d SET d_ytd = d_ytd - %d WHERE d_id = %d;", table_num, update_amount, d_id))
  -- Variation 1: END
...
--  WHERE w_id =:w_id
  ...
  -- Variation 2: BEGIN
    local new_carrier_id = sysbench.rand.uniform(1, 10)  -- Must be within 1-10
    con:query(string.format("UPDATE /* Variation2 */ orders%d SET o_carrier_id = %d WHERE o_id = %d AND o_w_id = %d;", table_num, new_carrier_id, sysbench.rand.uniform(1, 100), w_id))
    
    con:query("SELECT SLEEP(1);")
    
    -- Ensure no_o_id does not cause duplicates or exceed limits
    local no_o_id_offset = sysbench.rand.uniform(1, 3)
    con:query(string.format("UPDATE /* Variation2 */ new_orders%d SET no_o_id = LEAST(no_o_id + %d, 99999) WHERE no_o_id < 100 AND no_d_id = %d AND no_w_id = %d;", table_num, no_o_id_offset, d_id, w_id))
  -- Variation 2: END  
...
function orderstatus()
    ...
    -- Variation 3: BEGIN
      local balance_change = sysbench.rand.uniform(-100, 100)

      local c_balance = con:query_row(string.format("SELECT c_balance FROM customer%d WHERE c_id = %d AND c_d_id = %d AND c_w_id = %d;", table_num, c_id, d_id, w_id))
      if c_balance == nil then c_balance = 0 end -- Prevent nil value

      if c_balance + balance_change < -2147483648 then balance_change = -2147483648 - c_balance end
      if c_balance + balance_change > 2147483647 then balance_change = 2147483647 - c_balance end

      con:query(string.format("UPDATE /* Variation3 */ customer%d SET c_balance = c_balance + %d WHERE c_id = %d AND c_d_id = %d AND c_w_id = %d;", table_num, balance_change, c_id, d_id, w_id))
      con:query("SELECT SLEEP(1);")

      con:query(string.format("UPDATE /* Variation3 */ warehouse%d SET w_ytd = w_ytd - %d WHERE w_id = %d;", table_num, balance_change, w_id))
    -- Variation 3: END
...
function delivery()
    ...
    for  d_id = 1, DIST_PER_WARE do
      ...
      -- Variation 4: BEGIN
        local s_quantity = con:query_row(string.format("SELECT s_quantity FROM stock%d WHERE s_i_id = %d AND s_w_id = %d", table_num, sysbench.rand.uniform(1, 100), w_id))
        if s_quantity == nil then s_quantity = 0 end -- Prevent nil value
        local delta_quantity = sysbench.rand.uniform(-10, 10)

        if s_quantity + delta_quantity < 0 then delta_quantity = -s_quantity end  -- Prevent negative stock
        if s_quantity + delta_quantity > 1000 then delta_quantity = 1000 - s_quantity end  -- Cap stock at 1000

        con:query(string.format("UPDATE /* Variation4 */ stock%d SET s_quantity = s_quantity + %d WHERE s_i_id = %d AND s_w_id = %d;", table_num, delta_quantity, sysbench.rand.uniform(1, 100), w_id))
  
        con:query("SELECT SLEEP(1);")

        local ol_quantity = con:query_row(string.format("SELECT ol_quantity FROM order_line%d WHERE ol_o_id = %d AND ol_w_id = %d", table_num, sysbench.rand.uniform(1, 100), w_id))
        if ol_quantity == nil then ol_quantity = 0 end -- Prevent nil value
        local delta_ol_quantity = sysbench.rand.uniform(-5, 5)

        if ol_quantity + delta_ol_quantity < 0 then delta_ol_quantity = -ol_quantity end  -- Prevent negative
        if ol_quantity + delta_ol_quantity > 100 then delta_ol_quantity = 100 - ol_quantity end  -- Cap at 100

        con:query(string.format("UPDATE /* Variation4 */ order_line%d SET ol_quantity = ol_quantity + %d WHERE ol_o_id = %d AND ol_w_id = %d;", table_num, delta_ol_quantity, sysbench.rand.uniform(1, 100), w_id))
      -- Variation 4: END
...
function stocklevel()
    ...
    -- Variation 5: BEGIN
      local stock_change = sysbench.rand.uniform(-50, 50)
      local item_id = sysbench.rand.uniform(1, 100)
  
      local s_quantity = con:query_row(string.format("SELECT s_quantity FROM stock%d WHERE s_i_id = %d AND s_w_id = %d;", table_num, item_id, w_id))
      if s_quantity == nil then s_quantity = 0 end  -- Prevent nil value
  
      if s_quantity + stock_change < 0 then stock_change = -s_quantity end
      if s_quantity + stock_change > 1000 then stock_change = 1000 - s_quantity end

      con:query(string.format("UPDATE /* Variation5 */ stock%d SET s_quantity = s_quantity + %d WHERE s_i_id = %d AND s_w_id = %d;", table_num, stock_change, item_id, w_id))

      con:query("SELECT SLEEP(1);")

      local price_update = sysbench.rand.uniform(-5, 5)
      local i_price = con:query_row(string.format("SELECT i_price FROM item%d WHERE i_id = %d;", table_num, item_id))
      if i_price == nil then i_price = 0 end  -- Prevent nil value

      if i_price + price_update < 0 then price_update = -i_price end  -- Ensure price doesn't go negative

      con:query(string.format("UPDATE /* Variation5 */ item%d SET i_price = i_price + %d WHERE i_id = %d;", table_num, price_update, item_id))    
    -- Variation 5: END    
...
function purge()
    ...
    -- Variation 6: BEGIN
      ...
      local no_o_id = con:query_row(string.format("SELECT MIN(no_o_id) FROM new_orders%d WHERE no_w_id = %d;", table_num, w_id))
      if no_o_id == nil then no_o_id = 0 end  -- Prevent nil value
      if no_o_id + no_o_id_offset < 1 then no_o_id_offset = 1 - no_o_id end  -- Ensure no_o_id is at least 1

      con:query(string.format("UPDATE /* Variation6 */ new_orders%d SET no_o_id = no_o_id + %d WHERE no_w_id = %d;", table_num, no_o_id_offset, w_id))
  
      -- con:query("SELECT SLEEP(1);")

      local o_id = con:query_row(string.format("SELECT MIN(o_id) FROM orders%d WHERE o_w_id = %d;", table_num, w_id))
      if o_id == nil then o_id = 0 end  -- Prevent nil value
      if o_id - 1 < 1 then o_id = 1 end  -- Ensure o_id never goes below 1

      con:query(string.format("UPDATE /* Variation6 */ orders%d SET o_id = o_id - 1 WHERE o_w_id = %d;", table_num, w_id))
    -- Variation 6: END   
